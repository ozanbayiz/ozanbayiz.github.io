...
After blurring, the gradient magnitude matrix was much less noisy, and the edges were better defined.
...
Then I created the Derivative of Gaussian (DoG) filters. The key insight is that convolution is associative, so blurring first and then differentiating is equivalent to convolving with the derivative of the Gaussian directly:

$$
\frac{\partial}{\partial x}(G * \text{img}) = \left(\frac{\partial G}{\partial x}\right) * \text{img}
$$

This means we can precompute $D_x G$ and $D_y G$ and apply them in a single convolution pass.

<Figure src="/projects/cs180/proj2/figures/DoG_filters.png" alt="DoG filters" caption="Derivative-of-Gaussian filters (Dx, Dy)." />

I got almost the same result. The DoG-convolved images had slightly more texture, possibly due to zero-padding differences.
...
- convert the high and low frequency ranges back into images using `np.fft.ifft2()`
